...
public class WrapByteArrayInputStream extends InputStream {
	 ...
	 * <code>offset+length</code>. The buffer array is not copied.
	 * <p>
	 * Note that if bytes are simply read from the resulting input stream,
	 * elements <code>buf[pos]</code> through <code>buf[pos+len-1]</code> will
	 * be read; however, if a <code>reset</code> operation is performed, then
	 * bytes <code>buf[0]</code> through b<code>uf[pos-1]</code> will then
